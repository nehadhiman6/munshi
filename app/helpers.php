    ...
    if ($date && $date != '0000-00-00' && $date != 'null') {
        if (is_object($date)) {
            if (get_class($date) == 'Carbon') {
                return $date->parse($date)->format('m-d-Y');
            } else {
                return $date;
            }
        }
...
function getDateTimeAttribute($date)
{
    if ($date && $date != '0000-00-00' && $date != 'null') {
        if (is_object($date)) {
            if (get_class($date) == 'Carbon') {
                return $date->parse($date)->format('m-d-Y h:i:s');
            } else {
                return $date;
            }
        }
...
function setStringAttribute($str)
{
    if (strlen($str) > 0) {
        return $str;
    }
...
{
    if ($prvYr) {
        $fy = session()->get('fy', '20202021');
        if ($fy != '') {
            $fy = (intval(substr($fy, -4)) - 2) . substr($fy, 0, 4);
        }
        return $fy;
    } else {
        return session()->get('fy', '20202021');
    }
}
