    ...
    <modal-cover @closeModal="checkUnsavedAndShowMessage"  :modal-hide="false" modal_size="modal-lg">
      <!-- MODAL BODY -->
        <div slot="modal-title">
            <span v-if="form.form_id == 0">Add <slot name="employee-type"></slot></span>
            <span v-if="form.form_id > 0">Edit <slot name="employee-type"></slot></span>
        </div>
            ...
            <div class="form-horizontal">
                <div class="card-body">
                    ...
                    <div class="form-row">
                         <div class="col">
                            <div class="form-group">
                                <label for="middle_name" class="in-label-title ">Initials</label>
                                <input class="form-control" v-bind:class="{ 'is-invalid': form.errors.get('name_initials') }" type="text" v-model ="form.name_initials">
                                <span id="basic-msg" v-if="form.errors.get('name_initials')" class="invalid-feedback">{{ form.errors.get('name_initials') }}</span>
                            </div>
                        </div>

                         <div class="col">
                            <div class="form-group">
                                <label for="middle_name" class="in-label-title ">Teraview Account Name</label>
                                <input class="form-control" v-bind:class="{ 'is-invalid': form.errors.get('teraview') }" type="text" v-model ="form.teraview">
                                <span id="basic-msg" v-if="form.errors.get('teraview')" class="invalid-feedback">{{ form.errors.get('teraview') }}</span>
                            </div>
                        </div>
            ...
            Utilities.showConfirmationPopMessage('Do you want to save the changes?',function(save){
                if(save == true){
                    ...
                    .then(function(response){
                        if(response.success){
                            Utilities.showToast(self,'success','Employee updated successfully',2200,true);
        ...
        editEmployee:function(){
            ...
            .then(function(response){
                if(response.data.success){
                    ...
                    Utilities.copyProperties(employee, self.form);
                    self.form.form_id = employee.id;
                    if(self.form.address == null){
                        self.form.address = getAddress();
                    }
                    if(employee.address && employee.address.city){
                        var city = employee.address.city;
