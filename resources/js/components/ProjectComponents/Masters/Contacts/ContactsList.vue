<template>
    <div>
        <contact-form v-if="formOpen"
                :permissions="permissions"
                :form-id ="form_id"
                @closeTriggered="reloadTable"
            >
            <div slot="title">
                  <span v-if="form_id == 0"> Add Mortgagee</span>
                  <span v-if="form_id > 0">Edit Mortgagee</span>
            </div>
        </contact-form>
        <div class="iw-sider-card-wrap m-box p-0">
                <div class="card-header">
                    ...
                    <a class="btn iw-btn-add  float-right ml-3"  v-show="formOpen == false && permissions['contacts-modify'] == 'contacts-modify'" @click.prevent="formOpen = !formOpen">
                        <i class="fe fe-plus"></i> New Mortagagee
                    </a>
                </div>
                <div class="card-body">
                    <table id="dt-contact-table" class="table table-striped table-bordered" width="100%">
                    </table>
                </div>
        </div>
    </div>
    ...
    export default{
        ...
        methods:{
            setTable:function(){
                var self = this;
                var target = 0;
                self.table = $('#dt-contact-table').DataTable({
                    "processing": true,
                    "serverSide": true,
                    "searchDelay": 700,
                    "ajax": {
                    ...
                    columnDefs: [
                        ...
                        { title: 'Company Name', targets: target++, data: 'company_name',
                        },
            ...
            checkPermission: function(){
                if(this.permissions['contacts-modify'] == 'contacts-modify'){
                    return true;
                }else{
                    return false;
                }
