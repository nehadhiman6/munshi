<template>
 <span >
    <select :class="'form-control ' + customClass +' solicitor'+index"  :disabled="disabled"></select>
</span>
</template>
...
export default {
    props:{
        ...
        url:{default:'solicitors/filtered',type: String},
    ...
    methods:{
        intialiseSelect:function(){
                var self = this;
                var itemComponent = $('.solicitor'+self.index);
                ...
                .on('select2:select', function (e) {
                    ...
                    var item =  parseInt($('.solicitor'+ self.index).val());
                    self.$emit('input', item);
                    $(".solicitor"+self.index+" option:selected").html();
                    // update item returns (value, index, acname, item)
                    self.$emit('updateSolicitor',item,self.index,self.selected_data);
                })


                if(typeof(this.index) !="undefined"){
                    // $('.acid'+self.index).val(self.selected).trigger('change');
                    $('.solicitor'+self.index).val(self.selected);
                }
            },
