<template>
 <span >
    <select :class="'form-control ' + customClass +' real_estate_agent_'+index"  :disabled="disabled"></select>
</span>
</template>
...
export default {
    props:{
        ...
        url:{default:'real-estate-agents/filtered',type: String},
    ...
    methods:{
        intialiseSelect:function(){
                var self = this;
                var itemComponent = $('.real_estate_agent_'+self.index);
                var item = itemComponent.select2({
                    dropdownAutoWidth : true,
                    placeholder: "Select Real Estate agent",
                    ...
                    ajax: {
                        ...
                        processResults: function (data,params) {
                            ...
                            data.results.forEach(function(ele){
                                ...
                                ele.text = first_name +' '+ middle_name + ' ' +last_name;
                            });
                ...
                .on('select2:select', function (e) {
                    ...
                    var item =  parseInt($('.real_estate_agent_'+ self.index).val());
                    self.$emit('input', item);
                    $(".real_estate_agent_"+self.index+" option:selected").html();
                    // update item returns (value, index, acname, item)
                    self.$emit('updateRealEstateAgent',item,self.index,self.selected_data);
                });

                if(typeof(this.index) !="undefined"){
                    // $('.acid'+self.index).val(self.selected).trigger('change');
                    $('.real_estate_agent_'+self.index).val(self.selected);
                }
            },
