<template>
 <span >
    <select :class="'form-control ' + customClass +' brokerage'+index"  :disabled="disabled"></select>
</span>
</template>
...
export default {
    props:{
        ...
        url:{default:'brokerages/filtered',type: String},
    ...
    methods:{
        intialiseSelect:function(){
                var self = this;
                var itemComponent = $('.brokerage'+self.index);
                ...
                .on('select2:select', function (e) {
                    ...
                    var item =  parseInt($('.brokerage'+ self.index).val());
                    self.$emit('input', item);
                    $(".brokerage"+self.index+" option:selected").html();
                    // update item returns (value, index, acname, item)
                    self.$emit('updateBroker',item,self.index,self.selected_data);
                });

                if(typeof(this.index) !="undefined"){
                    // $('.acid'+self.index).val(self.selected).trigger('change');
                    $('.brokerage'+self.index).val(self.selected);
                }
            },
            setName :function(details) {
                    if (!details.id) { return details.text; }
                    var $details = $('<div class="row"><div class="col-md-7">' + details.text + '</div></div>');
                    return $details;
            },
