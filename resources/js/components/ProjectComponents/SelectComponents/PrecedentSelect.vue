<template>
 <span >
    <select :class="'form-control ' + customClass +' precedent_'+index"  :disabled="disabled"></select>
</span>
</template>
...
export default {
    props:{
        ...
        url:{default:'precedents/filtered',type: String},
    ...
    methods:{
        intialiseSelect:function(){
                var self = this;
                var itemComponent = $('.precedent_'+self.index);
                ...
                .on('select2:select', function (e) {
                    ...
                    var item =  parseInt($('.precedent_'+ self.index).val());
                    if(self.stringValue){
                    ...
                    $(".precedent_"+self.index+" option:selected").html();
                    // update item returns (value, index, acname, item)
                    self.$emit('updatePrecedent',item,self.index,self.selected_data);
                });

                if(typeof(this.index) !="undefined"){
                    // $('.acid'+self.index).val(self.selected).trigger('change');
                    $('.precedent_'+self.index).val(self.selected);
                }
            },
