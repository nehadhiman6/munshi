<template>
 <span >
    <select :class="'form-control ' + customClass +' employee'+index"  :disabled="disabled"></select>
</span>
</template>
...
export default {
    props:{
        ...
        url:{default:'employees/filtered',type: String},
    ...
    methods:{
        intialiseSelect:function(){
                var self = this;
                var itemComponent = $('.employee'+self.index);
                ...
                .on('select2:select', function (e) {
                    ...
                    var item =  parseInt($('.employee'+ self.index).val());
                    self.$emit('input', item);
                    $(".employee"+self.index+" option:selected").html();
                    // update item returns (value, index, acname, item)
                    self.$emit('updateFirm',item,self.index,self.selected_data);
                });

                if(typeof(this.index) !="undefined"){
                    // $('.acid'+self.index).val(self.selected).trigger('change');
                    $('.employee'+self.index).val(self.selected);
                }
            },
